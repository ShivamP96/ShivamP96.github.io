---
name: Score Tracker
tools: [TypeScript, ReactJS, Sports]
image: /assets/img/score-tracker.gif
description: Simple Score Tracker made with TS and React to give you the ablity to keep count when playing sports with others. Utilizes local storage.
---

# Score Tracker

The Score Tracker Project is a simple tracker for when you want to play 1 on 1 sports or team 1 on 1 sports!
Use this Project to your own liking!

![preview](/assets/img/score-tracker.gif)

## Tech Stack

    - TypeScript

## Motivation

I built this WebApp to create a personalized way to track game scores while playing volleyball with my friends. It provides a simple and efficient way to keep score, ensuring a seamless playing experience.

<p class="text-center">
{% include elements/button.html link="https://github.com/ShivamP96/ScoreTracker" text="Learn More" %}
</p>
